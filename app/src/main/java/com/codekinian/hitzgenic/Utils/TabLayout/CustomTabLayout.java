package com.codekinian.hitzgenic.Utils.TabLayout;
...
public class CustomTabLayout extends HorizontalScrollView {
    ...
    public CustomTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.CustomTabLayout_tabIndicatorHeight, 0));
        // default value was 28dp
        mTabStrip.setSelectedIndicatorWidth(
                a.getDimensionPixelSize(R.styleable.CustomTabLayout_tabIndicatorWidth, dpToPx(28)));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.CustomTabLayout_tabIndicatorColor, 0));

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.CustomTabLayout_tabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.CustomTabLayout_tabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.CustomTabLayout_tabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.CustomTabLayout_tabPaddingEnd,
                mTabPaddingEnd);
        ...
        if (a.hasValue(R.styleable.CustomTabLayout_tabTextColor)) {
            // If we have an explicit text color set, use it instead
            mTabTextColors = a.getColorStateList(R.styleable.CustomTabLayout_tabTextColor);
        }

        if (a.hasValue(R.styleable.CustomTabLayout_tabSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.CustomTabLayout_tabSelectedTextColor, 0);
            mTabTextColors = createColorStateList(mTabTextColors.getDefaultColor(), selected);
        }

        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.CustomTabLayout_tabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.CustomTabLayout_tabMaxWidth,
                INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.CustomTabLayout_tabBackground, 0);
        mContentInsetStart = a.getDimensionPixelSize(R.styleable.CustomTabLayout_tabContentStart, 0);
        mMode = a.getInt(R.styleable.CustomTabLayout_tabMode, MODE_FIXED);
        mTabGravity = a.getInt(R.styleable.CustomTabLayout_tabGravity, GRAVITY_FILL);
        a.recycle();
